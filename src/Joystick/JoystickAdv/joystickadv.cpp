...
JoystickAdv::JoystickAdv(AsyncIOStream & iface) :
		_iface(iface)
{
	using namespace std::placeholders;
	_iface.read = std::bind(&JoystickAdv::read, this, _1, _2);
...
void JoystickAdv::read(const uint8_t * inData, uint32_t byteToRead)
{
	/*outputs*/
			...
			InputReadingByte++)
	{
		...
		{
		case $55Byte:
			if (inData[InputReadingByte] != 0x55)
				OutReadingByte = 0;
			else
				OutReadingByte++;
			break;
		...
		case $FCByte:
			if (inData[InputReadingByte] != 0xFC)
				OutReadingByte = 0;
			else
				OutReadingByte++;
			break;
		...
		case LSB_Checksum_byte:
			...
			for (unsigned char i = 0; i < sizeof(JoyData); i++)
				CalcCRC += ((unsigned char*) &joydata)[i];
			if (CmdCRC == CalcCRC)
				dataRsv(joydata);
			OutReadingByte = 0;
			break;
		...
		default:
			((char*) &joydata)[dataByte - (OutReadingByte - StartData_byte)] =
