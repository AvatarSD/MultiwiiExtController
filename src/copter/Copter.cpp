...
Copter::Copter(): MachineControl(), MultiWii()
{
	_accelZeroGravity = 0;
...
void Copter::setArm(){_controlData.arm_ch = armed;}

void Copter::setDisarm(){_controlData.arm_ch = disarmed;}

char Copter::isArmed(){
	if(_controlData.arm_ch == armed) return 1;
	else if(_controlData.arm_ch == disarmed) return 0;
	else return -1;
}
